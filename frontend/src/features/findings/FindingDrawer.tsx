...
import { useFinding, useRemediation } from "@/lib/hooks";
...
/** LLM-shaped fields are typed as string but not guaranteed to be one at
 *  runtime — rendering an object/array directly as a child crashes React. */
function asText(v: unknown): string {
  if (v == null) return "";
  if (typeof v === "string") return v;
  if (typeof v === "object") return Object.values(v as object).map(asText).join("; ");
  return String(v);
}
...
export default function FindingDrawer({ qid, onClose }: { qid: number | null; onClose: () => void }) {
  ...
  const cached = useRemediation(qid);
  const rem = useMutation<Remediation>({
    mutationFn: () => api.remediation(qid!),
    onSuccess: (d) => {
      if (!d.error) toast.success("Remediation plan ready");
      queryClient.setQueryData(["remediation", qid], d);
    },
  });

  // A plan already generated in a previous session (fetched via GET) counts
  // the same as one just generated by the mutation — either way, the button
  // should give way to the plan instead of offering to regenerate it.
  const plan = rem.data ?? (cached.data?.generated === false ? undefined : cached.data);
          ...
          <div className="mt-6">
            {!hasPlan && (
              <button
                ...
                onClick={() => rem.mutate()}
                disabled={rem.isPending || cached.isLoading}
              >
            ...
            {plan?.error && (
              <p className="mt-2 text-sm text-immediate">{plan.error}</p>
            )}
            {hasPlan && plan && (
              <ErrorBoundary fallbackTitle="Couldn't render this remediation plan.">
                <div className="mt-2 space-y-4 rounded-xl border border-sage/20 bg-sage/[0.06] p-4">
                  <div>
                    <AiBadge>Remediation Agent</AiBadge>
                    <p className="text-sm leading-relaxed text-ink-muted">{asText(plan.analyst_summary)}</p>
                  </div>
                  {Array.isArray(plan.step_by_step) && plan.step_by_step.length > 0 && (
                    <div>
                      <div className="mb-1.5 flex items-center gap-1.5 text-sm font-semibold text-ink">
                        <Wrench size={14} /> Steps
                      </div>
                      <ol className="space-y-1.5">
                        {plan.step_by_step.map((s, i) => (
                  ...
                  {Array.isArray(plan.validation_steps) && plan.validation_steps.length > 0 && (
                    <div>
                      <div className="mb-1.5 flex items-center gap-1.5 text-sm font-semibold text-ink">
                        <CheckCircle2 size={14} /> Validation
                      </div>
                      <ul className="space-y-1">
                        {plan.validation_steps.map((s, i) => (
                          <li key={i} className="text-sm text-ink-muted">· {String(s)}</li>
                        ))}
                      </ul>
                    </div>
                  )}
                  <div className="flex flex-wrap gap-2 pt-1">
                    {plan.risk_of_fix && <Tag>Risk: {asText(plan.risk_of_fix)}</Tag>}
                    {plan.estimated_effort && <Tag color="rgb(var(--c-sage-bright))">{asText(plan.estimated_effort)}</Tag>}
                  </div>
                </div>
              </ErrorBoundary>
            )}
