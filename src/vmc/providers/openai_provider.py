"""OpenAI adapter. Uses native JSON mode via `response_format`; the shared
repair loop is still the safety net for models/prompts that don't honor it
strictly.
"""
...
class OpenAIProvider:
    ...
    def _ensure_client(self):
        ...
        try:
            ...
            raise ProviderError(
                ...
                "openai is not installed (pip install vmc[openai])",
                provider=self.name,
                cause=exc,
            ) from exc
    ...
    async def _raw_generate(self, system: str, prompt: str, temperature: float) -> str:
        ...
        try:
            ...
            raise _normalize_openai_error(exc, self.name) from exc
    ...
    async def generate_with_vision(
        ...
        *,
        ...
        raise ProviderError(
            ProviderErrorType.UNKNOWN,
            "OpenAIProvider has no vision model configured; route vision calls to gemini or anthropic",
            provider=self.name,
        )


def _normalize_openai_error(exc: Exception, provider_name: str) -> ProviderError:
